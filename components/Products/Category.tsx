...
const categories = [
  { label: "Spirits", Icon: FiStar },
  { label: "Beer", Icon: FiCoffee },
  { label: "Wine", Icon: FiStar },
  { label: "Ciders & Seltzers", Icon: FiPackage },
  { label: "Mix & Chips", Icon: FiShoppingBag },
  { label: "Non-Alcohol", Icon: FiShield },
] as const;
    ...
    <section className="relative w-full overflow-hidden bg-gradient-to-b from-violet-900 via-purple-900 to-fuchsia-950 py-10">
      ...
      <SectionWrapper className="relative py-0">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
          {categories.map(({ label, Icon }) => (
            <button
              key={label}
              type="button"
              className="inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-xs font-medium uppercase tracking-wide text-white transition-colors hover:bg-white/20"
            >
        ...
        <div className="mt-3 flex justify-center">
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-wide text-white transition-colors hover:bg-white/20"
          >
